...
const functionalErrorNames = {
    ...
    userNoExists: { errorCode: 3, message: 'User doesnt exists', userMessage: 'Unauthorized user' },
    userNotActive: { errorCode: 4, message: 'User is not active or invalid password', userMessage: 'Unauthorized user' },
    badCredentials: { errorCode: 5, message: 'Bad login credentials or password', userMessage: 'Unauthorized user' },
    tokenNoExists: { errorCode: 6, message: 'Token doesnt exists', userMessage: 'Unauthorized user' },
    invalidAccessToken: { errorCode: 7, message: 'Could not verify user access token', userMessage: 'Unauthorized user' },
    ...
    twoUsersSameEmail: { errorCode: 70, message: 'Two or more users with the same email', userMessage: 'Server error' },
    usersNotAuthorized: { errorCode: 71, message: 'Unauthorized user to Admin application', userMessage: 'Unauthorized user' },
};


class FuncionalError {

    constructor(err, data, processName, functionName) {

        const e = functionalErrorNames[err] || functionalErrorNames['genericError']


        this.name = err;
        this.typeError = 'Functional error';
        this.error = (e.errorCode) ? e.errorCode : "0";
        this.message = e.message;
        this.data = data;
        this.userMessage = (e.userMessage) ? e.userMessage : "Aplication error";
...
module.exports.FuncError = FuncionalError;